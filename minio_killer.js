...
function check() {
  ...
  if (!isAlive(parentPid)) {
    console.log('minio_killer: parentPid not alive');
    try {
      process.kill(childPid);
    } catch (err) {
      console.log('minio_killer: could not kill child:', err);
    }
    process.exit();
  }
  ...
  // if child dead
  if (!isAlive(childPid)) {
    console.log('minio_killer: childPid not alive');
    process.exit();
  }
}

if (isNullOrUndefined(parentPid) || Number.isNaN(parentPid)) {
  console.log('minio_killer: parentPid is undefined or NaN!');
  process.exit(-1);
}

if (isNullOrUndefined(childPid) || Number.isNaN(childPid)) {
  console.log('minio_killer: childPid is undefined or NaN!');
  process.exit(-1);
}
